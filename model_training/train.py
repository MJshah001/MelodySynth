...
SAVE_MODEL_DIR_PATH = "../models"
SAVE_LEARNING_CURVE_DIR_PATH = "./learning_curves"
MAPPING_PATH = "../data_preprocessing/mapping.json" # path to the json file
...
def train(output_units=OUTPUT_UNITS, num_units=NUM_UNITS, loss=LOSS, learning_rate=LEARNING_RATE,model_name="LSTM",validation_split=0.1):
    ...
    if model_name == 'LSTM':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"lstm_model.h5")
    ...
    if model_name == 'Bi-LSTM':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"bi_lstm_model.h5")
      model = build_model_bi_lstm(output_units, num_units, loss, learning_rate)

    if model_name == 'Stacked-LSTM':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"stacked_lstm_model.h5")
      model = build_model_stacked_lstm(output_units, num_units, loss, learning_rate)

    if model_name == 'GRU':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"gru_model.h5")
      model = build_model_gru(output_units, num_units, loss, learning_rate)

    if model_name == 'Bi-GRU':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"bi_gru_model.h5")
      model = build_model_bi_gru(output_units, num_units, loss, learning_rate)

    if model_name == 'RNN':
      SAVE_MODEL_PATH = os.path.join(SAVE_MODEL_DIR_PATH,"simple_rnn_model.h5")
...
def plot_training_history(history, model_name):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Value')
    plt.title(model_name + ' Training Loss and Accuracy')
    plt.legend()
    
    # creating SAVE_LEARNING_CURVE_DIR_PATH if it does not exist
    if not os.path.exists(SAVE_LEARNING_CURVE_DIR_PATH):
        os.mkdir(SAVE_LEARNING_CURVE_DIR_PATH)

    # Saving the plot as an image
    image_path = os.path.join(SAVE_LEARNING_CURVE_DIR_PATH,f"{model_name}_learning_curve.png")
...
if __name__ == "__main__":
   ...
   try:
        ...
        if 1 <= model_index <= len(model_names):
            ...
            print(f"\n\nTraining completed. The learning curve for {model_name} model is saved as an image in learning_curves folder.")
